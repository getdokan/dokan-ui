      ...
      <div className={classNames(`rounded-md ${bgColors[props.color]} p-4`, props.className)}>
        <div className={`flex ${props.centered ? 'justify-center' : ''}`}>
          <div className="flex-shrink-0">
            ...
            {props.type == 'warning' && (
              <HiExclamation className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />
            )}
            {props.type == 'danger' && (
              <HiXCircle className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />
            )}
            {props.type == 'info' && (
              <HiInformationCircle className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />
            )}
          </div>
          <div className="ml-3">
