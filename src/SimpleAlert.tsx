        ...
        <div className={`rounded-md ${bgColors[props.color]} p-4`}>
            <div className={`flex ${props.centered ? 'justify-center' : ''}`}>
                <div className="flex-shrink-0">
                    {props.type == 'success' && <CheckCircleIcon className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />}
                    {props.type == 'warning' && <ExclamationIcon className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />}
                    {props.type == 'danger' && <XCircleIcon className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />}
                    {props.type == 'info' && <InformationCircleIcon className={`h-5 w-5 ${textColors400[props.color]}`} aria-hidden="true" />}
                </div>
                <div className="ml-3">
                    <p className={`text-sm font-medium ${textColors800[props.color]}`}>{props.label}</p>
                </div>
