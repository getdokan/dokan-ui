...
const TrailingInput: React.FC<TrailingInputProps> = (props) => {
    ...
    let validClasses = `focus:ring-primary-500 focus:border-primary-500 block w-full rounded-none rounded-l ${props.icon && 'pl-10'} sm:text-sm border-gray-300 placeholder-gray-400`;
    ...
    return (
        <>
            ...
            {props.errors && <p className="text-xs text-red-600" id={`${props.input.id}-error`}>
                {props.errors}
            </p>}
        </>
    );
